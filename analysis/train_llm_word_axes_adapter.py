"""
Train LLM adapter to MEG word-level semantic axes (ICA/PCA scores).

This maps word-level hidden states (averaged by word type across stories)
to MEG word-axis scores (e.g., ICA axes from outputs/word_axes_ica).
"""
...
def main():
    p = argparse.ArgumentParser(description="Train LLM adapter to MEG word axes.")
    ...
    p.add_argument("--axes", nargs="+", type=int, default=None, help="Axis indices to use (default: all).")
    p.add_argument("--stories", nargs="+", type=int, default=None, help="Story IDs (default: 1..60).")
    p.add_argument("--train-frac", type=float, default=0.8)
    p.add_argument("--min-count", type=int, default=1, help="Min occurrences per word.")
    ...
    print(f"[saved] adapter to {args.out_dir} (matched {len(matched_words)} words)")
